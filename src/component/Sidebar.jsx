...
const Sidebar = ({isMenuOpen, setIsMenuOpen}) => {
  ...
  const sidebarClasses = isMenuOpen ? 'max-md:translate-x-0' : 'max-md:-translate-x-full';
  ...
  return (
    ...
    transition-all duration-500 max-md:absolute left-0 z-10 ${sidebarClasses}`}>
      {/*logo*/}
     ...
     dark:border-white/20 rounded-md'>
        <img src={assets.search_icon} className='' alt='w-4 not-dark:invert' />
      ...
      dark:border-white/15 rounded-md cursor-pointer hover:scale-13 trasition-all'>
        ...
        <div className='flex flex-col text-sm'>
          ...
          <p className='text-xs text-gray-400'>Purchase credit to use quickhgpt</p>
        </div>
      </div>
      ...
      <img onClick={() => setIsMenuOpen(false)} src={assets.close_icon} className='absolute top-3 
      right-3 w-5 h-5 cursor-pointer md:hidden not-dark:invert' alt=''/>
