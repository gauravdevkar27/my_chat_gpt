...
import { dummyPublishedImages } from '../main_assets/assets'
  ...
  return (
    ...
    {images.length > 0 ? (
      <div className='flex flex-wrap max-sm:justify-center gap-5'>
        {images.map((item, idx)=>(
          <a key={idx} href={item.imageUrl} target='_blank' rel="noopener noreferrer" className='relative group block rounded-lg
          overflow-hidden border border-gray-200 dark:border-purple-700 shadow-sm
          hover:shadow-md transition-shadow duration-300'>
            <img src={item.imageUrl} alt={item.userName} className='w-full h-40 md:h-52
            2xl:h-64 object-cover group-hover:scale-105 transition-transform duration-300
            ease-in-out'/>
